...
BOOTSTRAP_MODULE = basstler.plan_item_bootstrap.__name__
"""
...
class RefusingPullRequestOpener:
    ...
    def open_pull_request(self, request: PullRequestRequest) -> CreatedPullRequest:
        ...
        """
        raise basstler.plan_item_bootstrap.PullRequestRefusedError(detail="422 refused")
...
def test_recording_a_new_item_without_a_title_names_the_key_it_needs(
    ...
    with pytest.raises(basstler.plan_item_bootstrap.IncompleteNewItemError) as refusal:
        record_item(
            record_request(
                bootstrap_repository,
                item_identifier=NEW_ITEM,
...
def test_opening_an_already_published_branch_is_refused(
    ...
    with pytest.raises(
        basstler.plan_item_bootstrap.BranchAlreadyPublishedError
    ) as refusal:
        open_work(
            open_request(),
            project_root=bootstrap_repository.project_root,
...
def test_a_refused_pull_request_leaves_the_manifest_untouched(
    ...
    with pytest.raises(basstler.plan_item_bootstrap.PullRequestRefusedError):
        open_work(
            open_request(),
            project_root=bootstrap_repository.project_root,
...
def test_creating_a_pull_request_without_a_title_or_body_is_refused_before_publishing(
    ...
    with pytest.raises(basstler.plan_item_bootstrap.PullRequestDetailsMissingError):
        open_work(
...
NARROWLY_INDENTED_MANIFEST = (
    DATASET_DIRECTORY / "narrowly-indented-plan.yaml"
).read_text()
"""
...
def test_each_refusal_carries_its_own_exit_code():
    codes = {
        ...
        basstler.plan_item_bootstrap.IncompleteNewItemError: ExitCode.INCOMPLETE_NEW_ITEM,
        basstler.plan_item_bootstrap.BranchAlreadyPublishedError: (
            ExitCode.BRANCH_ALREADY_PUBLISHED
        ),
        basstler.plan_item_bootstrap.PullRequestDetailsMissingError: (
            ExitCode.PULL_REQUEST_DETAILS_MISSING
        ),
        basstler.plan_item_bootstrap.PullRequestRefusedError: ExitCode.PULL_REQUEST_REFUSED,
    }
