...
def test_the_module_writes_the_scheme_once() -> None:
    ...
    """
    source = Path(basstler.setup_steps.__file__).read_text(encoding="utf-8")
...
def test_running_the_module_prints_the_steps_for_its_own_clone(
    clone_with_fork_remote: ScratchRepository,
) -> None:
    """
    The module describes the clone it was imported from, not the one running the test.

    The scratch clone's ``origin`` names a fork this repository does not, so a module
    reading the wrong root prints a different checklist rather than the same one.
    """
