...
from .upstream_reviews_replay import (
    RecordedCall,
    RecordedResponse,
    ReplayingClient,
)
...
def recorded_thread(
    fixture: RecordedResponse, identifier: ThreadIdentifier
) -> ReviewThread:
    """
...
def test_every_recorded_thread_survives_the_read_unchanged(paginated_client):
    ...
    assert snapshot.thread(ThreadIdentifier.RESOLVED) == recorded_thread(
        RecordedResponse.PULL_REQUEST_PAGE_ONE, ThreadIdentifier.RESOLVED
    )
    assert snapshot.thread(ThreadIdentifier.UNRESOLVED_OUTDATED) == recorded_thread(
        RecordedResponse.PULL_REQUEST_PAGE_TWO, ThreadIdentifier.UNRESOLVED_OUTDATED
    )
...
def test_the_snapshot_identifies_the_pull_request_it_read(paginated_client):
    ...
    recorded = RecordedResponse.PULL_REQUEST_PAGE_ONE.recorded().pull_request_reviews
...
def test_a_thread_keeps_every_comment_in_order(paginated_client):
    ...
    recorded = recorded_thread(
        RecordedResponse.PULL_REQUEST_PAGE_ONE, ThreadIdentifier.UNRESOLVED_MIDDLE
    )
...
def test_a_branch_resolves_to_the_forks_own_pull_request():
    client = ReplayingClient([RecordedResponse.BRANCH_PULL_REQUESTS.load()])
...
def test_a_branch_of_another_contributor_is_not_claimed():
    client = ReplayingClient(
        [RecordedResponse.BRANCH_PULL_REQUESTS_FOREIGN_OWNER.load()]
    )
...
def test_a_branch_never_promoted_upstream_is_reported_clearly():
    client = ReplayingClient([RecordedResponse.BRANCH_PULL_REQUESTS_NONE.load()])
...
def stubbed_gh(tmp_path, monkeypatch) -> Path:
    ...
    shutil.copy(Path(__file__).parent / "dataset" / "stubs" / "gh.sh", installed)
...
def test_a_branch_without_an_upstream_pull_request_exits_without_a_traceback(
    ...
    monkeypatch.setenv(
        StubEnvironmentVariable.GRAPHQL_JSON,
        json.dumps(
            {PullRequestJSONKey.DATA: RecordedResponse.BRANCH_PULL_REQUESTS_NONE.load()}
        ),
    )
