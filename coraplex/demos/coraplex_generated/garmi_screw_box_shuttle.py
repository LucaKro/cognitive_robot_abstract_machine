"""
The Garmi carrying machine parts across a warehouse, from where they are stored to where
they are wanted.

Each leg is a :class:`~coraplex.robot_plans.actions.composite.transporting.TransportAction`,
which searches the floor around a part, and around where it is going, for somewhere the
arm reaches from. Only the parts and their two poses are written down here; where the
robot stands to reach them is not.

Every pose is one cast onto the hall's own geometry, so a part rests on a surface that
is really there rather than hovering over one. The parts differ in shape, and so do the
grasps: which face the hand closes on is what decides whether a part fits between the
fingers at all.
